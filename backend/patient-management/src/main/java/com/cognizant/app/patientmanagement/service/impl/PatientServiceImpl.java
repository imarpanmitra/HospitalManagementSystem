...
public class PatientServiceImpl implements PatientService {
	...
	@Override
	public MappingJacksonValue retrieveAllPatientsOfSpecificHospital(Long hospitalId) throws ResourceNotFoundException {
	...
	public ResponseEntity<Void> addNewPatient(PatientPojo patient, Long hospitalId) {
		...
		patient.setPatientFirstName(
				patient.getPatientFirstName().substring(0, 1).toUpperCase() + patient.getPatientFirstName().substring(1));
		patient.setPatientLastName(
				patient.getPatientLastName().substring(0, 1).toUpperCase() + patient.getPatientLastName().substring(1));
	...
	public ResponseEntity<Void> updatePatient(PatientPojo patient, Long hospitalId, Integer patientId) {
		...
		specificPatient.setPatientFirstName(
				patient.getPatientFirstName().substring(0, 1).toUpperCase() + patient.getPatientFirstName().substring(1));
		specificPatient.setPatientLastName(
				patient.getPatientLastName().substring(0, 1).toUpperCase() + patient.getPatientLastName().substring(1));
